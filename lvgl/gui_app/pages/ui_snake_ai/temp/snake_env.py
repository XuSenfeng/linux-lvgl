...
class Game:
    ...
    # 可以使用的位置检测蛇是否撞到自己身体或墙壁
    def head_hit_body(self,position=None):
        if position is None:
            position = self.snake.blocks[0] # 蛇头
    ...
    def get_state(self):
        ...
        # 记录多步内是否会撞到身体, 上下左右四个方向里面是否有身体
        points_l = [Position(i, head.y) for i in range(1,head.x)] # 左侧所有点
